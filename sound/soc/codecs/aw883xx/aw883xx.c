...
static void *aw883xx_devm_kstrdup(struct device *dev, char *buf)
{
	...
	if (!str) {
		printk("devm_kzalloc %s failed", buf);
		return str;
	}
				...
				const struct i2c_device_id *id)
{
    ...
    printk("aw883xx_i2c_probe start \n");
	if (!i2c_check_functionality(i2c->adapter, I2C_FUNC_I2C)) {
		aw_dev_err(&i2c->dev, "check_functionality failed");
		return -EIO;
	}
    printk("aw883xx_malloc_init\n");
    ...
    printk("aw883xx_hw_reset sucess\n");
	/* aw883xx chip id */
    ...
    printk("aw883xx_read_chipid sucess\n");
...
static int __init aw883xx_i2c_init(void)
{
	...
	printk("aw883xx driver version %s", AW883XX_DRIVER_VERSION);

	ret = i2c_add_driver(&aw883xx_i2c_driver);
	if (ret)
		printk("fail to add aw883xx device into i2c");
