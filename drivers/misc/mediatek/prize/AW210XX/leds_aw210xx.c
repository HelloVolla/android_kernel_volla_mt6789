...
static ssize_t gps_lna_store(struct class *class, struct class_attribute *attr,	const char *buf, size_t count)
{
	...
	int rc;
	unsigned int val = 0;
	struct aw210xx *aw210xx = g_aw210xx;
	
	if(aw210xx == NULL){
		pr_err("%s, aw210xx is null.\n", __func__);
		return count;
	}
...
static struct class_attribute hd8040_class_attrs[] = {
	__ATTR(effect, S_IRUGO | S_IWUSR, NULL, gps_lna_store),
	__ATTR_NULL,
};
...
static int hd8040_sysfs_create(void)
{
	...
	hd8040_class = class_create(THIS_MODULE, "aw210xx_led");
...
static void  hd8040_sysfs_destroy(void)
{
	int i = 0;

	for (i = 0; hd8040_class_attrs[i].attr.name; i++)
		class_remove_file(hd8040_class, &hd8040_class_attrs[i]);
