...
extern  int  fm78100b_driver_init(void);
extern void  fm78100b_driver_exit(void);
extern  int  fm78100_driver_init(void);
extern void  fm78100_driver_exit(void);
/* exergen probe */
static int exergen_probe(struct platform_device *pdev)
{
	int exergen_type = 1;
	...
	ret=fm78100b_driver_init();
	if(ret<0){
		printk("lpp---typec_accdet_init failed\n");
		fm78100b_driver_exit();
	}
	ret=fm78100_driver_init();
	if(ret<0){
		printk("lpp---typec_accdet_init failed\n");
		fm78100_driver_exit();
	}
