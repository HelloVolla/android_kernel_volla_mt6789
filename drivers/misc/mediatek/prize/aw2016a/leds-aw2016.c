...
static int aw2016_get_boot_mode(struct aw2016_led *led_array)
{
	...
	union power_supply_propval val = {0,};
	 ...
	 if (!led_array->pdpe_psy) {
        led_array->pdpe_psy = power_supply_get_by_name("pdpe-state");
        if (!led_array->pdpe_psy) {
            pr_err("gezi get led_array->pdpe_psy failed---%s\n",__func__);
			return -ENODEV;
        }
    }
	
	ret = power_supply_get_property(led_array->pdpe_psy, POWER_SUPPLY_PROP_TEMP, &val);
	if (!ret){
		ret = val.intval;
		pr_err("gezi get---%s=%d\n",__func__,ret);
	}
