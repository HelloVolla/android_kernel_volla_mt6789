...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.i2c_addr_table = {0x5a, 0x20, 0x21, 0xff},
	.i2c_speed = 400,
};
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		...
		for( i = 0 ; i < 1000 ; i++)
		{
			...
			isStreamOn = read_cmos_sensor(0x0100); /* waiting for sensor to  stop output  then  set the  setting */
	...
	do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("[s5k4h7_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("[s5k4h7_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
