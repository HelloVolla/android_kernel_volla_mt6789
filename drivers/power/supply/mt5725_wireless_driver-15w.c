...
static char mt5725_init_done = 0;
...
static DEVICE_ATTR(wireless_connect, 0644, wireless_connect_show, NULL);
...
//#if defined(CONFIG_PRIZE_ONLY5W_DISPLAY_15W)
		...
		if(mte->charge_protocol == EPP  && mte->wireless_max_power >= 15){
			mte->input_current  = 1500000;
			mte->charge_current = 2300000;
		}
		else if(mte->charge_protocol == EPP && mte->wireless_max_power < 15){
			mte->input_current  = 1100000;
...
//prize add  by lipengpeng 20220611 end 
    pr_err("MT5725 chip.\n");
    chip->regmap = devm_regmap_init_i2c(client, &MT5725_regmap_config);
    if (!chip->regmap) {
...
//prize add by lipengpeng 20220623 start 
	...
	if (rc){
		pr_err(KERN_ERR"mt5725 failed device_create_file(dev_attr_wireless_connect)\n");
	}
//prize add by lipengpeng 20220623 end 
