...
void ili_report_ap_mode(u8 *buf, int len)
{
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		...
		//prize ---hjw---for 180--start
		if(touch_info[ilits->finger].x < 5)
			touch_info[ilits->finger].x = 5;
		
		//prize ---hjw---for 180--end
...
void ili_debug_mode_report_point(u8 *buf, int len)
{
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		...
		if(touch_info[ilits->finger].x < 20)
			touch_info[ilits->finger].x = 20;
		if(touch_info[ilits->finger].y < 20)
			touch_info[ilits->finger].y = 20;
		if(touch_info[ilits->finger].x > 2028)
			touch_info[ilits->finger].x = 2028;
		if(touch_info[ilits->finger].y > 2028)
			touch_info[ilits->finger].y = 2028;
