...
int ili_input_register(void)     //prize
                                 ...
                                 unsigned long event, void *data)
{
	...
	if (ilitek_data && data) {
		...
		printk("ilitek_plat_notifier_fb OUT");
	} else {
...
static void ilitek_plat_sleep_init(void)
{
    ...
    ilits->notifier_fb.notifier_call = ilitek_plat_notifier_fb;
//prize add by lipengpeng 20220525 start
        ret = mtk_disp_notifier_register("ilitek_v3 Touch", &ilits->notifier_fb);
        if (ret)
