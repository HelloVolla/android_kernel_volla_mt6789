...
int tpd_get_gpio_info(struct platform_device *pdev)
{
	...
	TPD_DEBUG("[tpd %d] mt_tpd_pinctrl+++++++++start++++++++\n", pdev->id);
	...
	if (IS_ERR(pins_default)) {
		...
		TPD_DEBUG("gezi ---- Cannot find pinctrl default %d!\n", ret);
	}
	...
	if (tpd_dts_data.tpd_use_ext_gpio == false) {
		...
		TPD_DEBUG("gezi get state_rst_output0 success\n");
		...
		TPD_DEBUG("gezi get state_rst_output1 success\n");
	}
	TPD_DEBUG("[tpd%d] mt_tpd_pinctrl-----end-----\n", pdev->id);
	return 0;
}
