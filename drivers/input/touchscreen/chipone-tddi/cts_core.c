...
void cts_get_glove_mode_reg(struct cts_device *cts_dev)
{
/*	
	int cts_fw_reg_readb_retry(const struct cts_device *cts_dev,
        u32 reg_addr, u8 *b, int retry, int delay)
*/
	...
	ret = cts_fw_reg_readb_retry(cts_dev,CTS_DEVICE_FW_REG_GLOVE_MODE,&value,1,10);
	
	if(ret < 0){
		pr_err("gezi cts_fw_reg_readb_retry err\n");
	}
	
	pr_err("%s--------%x",__func__,value);
...
int cts_resume_device(struct cts_device *cts_dev)
{
    ...
    if (cts_is_glove_enabled(cts_dev)) {
		...
		cts_get_glove_mode_reg(cts_dev);
    }
#endif
...
#if IS_ENABLED(CONFIG_PRIZE_SMART_COVER_COMMON_NODE)
...
static ssize_t fts_touch_glove_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(g_cts_dev == NULL){
		pr_err("gezi---g_cts_dev--err-----%s-----------\n",__func__);
		return 0;
	}
	
	if(buf == NULL){
		pr_err("gezi----buf-err-----%s-----------\n",__func__);
		return 0;
	}
	
	
    //count = sprintf(buf, "Glove Mode: %s\n", g_cts_dev->rtdata.glove_mode_enabled ? "On" : "Off");
...
static ssize_t fts_touch_glove_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int ret;
    int s = (buf[0] - '0');
    struct cts_device *cts_dev = g_cts_dev;
    ...
    if (s){
        if (!g_cts_dev->rtdata.glove_mode_enabled) {
            printk("[Mode]enter glove mode");
            ret = cts_enter_glove_mode(cts_dev);
        }
	}else{
            if (g_cts_dev->rtdata.glove_mode_enabled) 
			{
                printk("[Mode]exit glove mode");
				ret = cts_exit_glove_mode(cts_dev);
			}
    }
    printk("[Mode]glove mode status:  %d", g_cts_dev->rtdata.glove_mode_enabled);
    return count;
}
...
static ssize_t show_cmd_charge_disable(struct class *class, struct class_attribute *attr,	char *buf)
{
   ...
   return sprintf(buf, "Glove Mode: %s\n", g_cts_dev->rtdata.glove_mode_enabled ? "On" : "Off");
...
static struct class_attribute hd8040_class_attrs[] = {
	__ATTR(state, S_IRUGO | S_IWUSR, show_cmd_charge_disable, store_cmd_charge_disable),
	__ATTR_NULL,
};
...
static int cmd_charge_disable_sysfs_create(void)
{
	...
	hd8040_class = class_create(THIS_MODULE, "smartcover");
...
int init_sys_node(struct cts_device *cts_dev)
{
...
/*
		...
		struct kobject *prize_glove_t = NULL;
        //int err = 0;
		...
		if (!prize_glove_t){
			prize_glove_t = kobject_create_and_add("common_node", prize_glove);
		}

		if(sysfs_create_file(prize_glove_t, &dev_attr_state.attr))
		{
            printk("%s:%d sys create fail!",__func__,__LINE__);
			return -1;
		}
		
        printk("%s:%d sys create success!",__func__,__LINE__);
