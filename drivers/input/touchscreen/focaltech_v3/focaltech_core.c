...
void fts_reset_proc_on(int delay){
	FTS_FUNC_ENTER();
...
EXPORT_SYMBOL(fts_reset_proc_on);

void fts_reset_proc_off(int delay){
	FTS_FUNC_ENTER();
...
EXPORT_SYMBOL(fts_reset_proc_off);
...
//prize  hjw
#if 0
static int prize_fts_pinctrl_init(struct fts_ts_data *ts)
{
    int ret = 0;

    ts->prize_pinctrl = devm_pinctrl_get(ts->dev);
    if (IS_ERR_OR_NULL(ts->prize_pinctrl)) {
        FTS_ERROR("Pin state[ctp_ts_i2c_clk_gpio] not found");
        ret = PTR_ERR(ts->prize_pinctrl);
        //goto err_pinctrl_get;
    }

    ts->pins_i2c_data = pinctrl_lookup_state(ts->prize_pinctrl, "ctp_ts_i2c_sda");
    if (IS_ERR_OR_NULL(ts->pins_i2c_data)) {
        FTS_ERROR("Pin state[ctp_ts_i2c_sda] not found");
        ret = PTR_ERR(ts->pins_i2c_data);
        //goto err_pinctrl_lookup;
    }

    ts->pins_i2c_data_gpio = pinctrl_lookup_state(ts->prize_pinctrl, "ctp_ts_i2c_sda_gpio");
    if (IS_ERR_OR_NULL(ts->pins_i2c_data_gpio)) {
        FTS_ERROR("Pin state[ctp_ts_i2c_sda_gpio] not found");
        ret = PTR_ERR(ts->pins_i2c_data_gpio);
        //goto err_pinctrl_lookup;
    }

    ts->pins_i2c_clk = pinctrl_lookup_state(ts->prize_pinctrl, "ctp_ts_i2c_clk");
    if (IS_ERR_OR_NULL(ts->pins_i2c_clk)) {
        FTS_ERROR("Pin state[ctp_ts_i2c_clk] not found");
        ret = PTR_ERR(ts->pins_i2c_clk);
		//goto err_pinctrl_lookup;
    }
	ts->pins_i2c_clk_gpio = pinctrl_lookup_state(ts->prize_pinctrl, "ctp_ts_i2c_clk_gpio");
    if (IS_ERR_OR_NULL(ts->pins_i2c_clk_gpio)) {
        FTS_ERROR("Pin state[ctp_ts_i2c_clk_gpio] not found");
        ret = PTR_ERR(ts->pins_i2c_clk_gpio);
        //goto err_pinctrl_lookup;
    }
    return 0;
...
int fts_pinctrl_select_i2c_gpio(int n)
{
    int ret = 0;
    if (fts_data->pins_i2c_data && fts_data->pins_i2c_data_gpio && fts_data->pins_i2c_clk && fts_data->pins_i2c_clk_gpio) {
		if(n)
		{
			ret = pinctrl_select_state(fts_data->prize_pinctrl, fts_data->pins_i2c_data);
			if (ret < 0) {
				FTS_ERROR("pins_i2c_data_gpio Set normal pin state error:%d", ret);
			}
			ret = pinctrl_select_state(fts_data->prize_pinctrl, fts_data->pins_i2c_clk);
			if (ret < 0) {
				FTS_ERROR("pins_i2c_data_gpio Set normal pin state error:%d", ret);
			}
		}
		...
		{
			ret = pinctrl_select_state(fts_data->prize_pinctrl, fts_data->pins_i2c_data_gpio);
			if (ret < 0) {
				FTS_ERROR("pins_i2c_data_gpio Set normal pin state error:%d", ret);
			}
			ret = pinctrl_select_state(fts_data->prize_pinctrl, fts_data->pins_i2c_clk_gpio);
			if (ret < 0) {
				FTS_ERROR("pins_i2c_data_gpio Set normal pin state error:%d", ret);
			}
		}
...
static int fb_notifier_callback(struct notifier_block *self,
                                 unsigned long event, void *data)
{
	//struct focaltech_ts_data *focaltech_data =container_of(self, struct focaltech_ts_data, notifier_fb);
	//int *evdata = (int *)data;
	int *evdata = (int *)data;
   // int *blank = NULL;
    struct fts_ts_data *ts_data = container_of(self, struct fts_ts_data,
	...
	if (ts_data && data) {
		printk("focaltech_plat_notifier_fb IN");
		...
		printk("focaltech_plat_notifier_fb OUT");
	} else {
		printk("focaltech touch IC can not suspend or resume");
		return -1;
	}
	return 0;
}
...
//prize add by lipengpeng 20210420 start 
//	ret = prize_fts_pinctrl_init(ts_data);
//    if (ret) {
//        FTS_ERROR("fail to prize_fts_pinctrl_init");
//        goto err_gpio_config;
 //   }
//prize add by lipengpeng 20210420 end 
#if FTS_POWER_SOURCE_CUST_EN
...
//	struct fts_ts_data *ts_data = fts_data;	
    ret = fts_write_reg(FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP_VALUE);
    if (ret < 0) {
        FTS_ERROR(" pzp Set TP to sleep mode fail, ret=%d!", ret);
    }
}
EXPORT_SYMBOL(fts_enter_low_power);
//prize add by lipengpeng 20210420 end 
