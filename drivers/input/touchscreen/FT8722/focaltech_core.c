...
int fts_wait_tp_to_valid(void)
{
    ...
    do {
        ...
        if ((idh == chip_idh) || (fts_check_cid(ts_data, idh) == 0)) {
            printk("TP Ready,Device ID:0x%02x", idh);
            return 0;
        } else
                         ...
                         u16 type, u8 id_h, u8 id_l, bool force)
{
    ...
    for (i = 0; i < cid_entries; i++) {
        if (!force && (type == chip_id_list[i].type)) {
            ...
            printk("match cid,type:0x%x", (int)chip_id_list[i].type);
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    printk("get ic information, chip id = 0x%02x%02x(cid type=0x%x)",
...
static int fts_read_parse_touchdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    if ((touch_buf[1] == 0xFF) && (touch_buf[2] == 0xFF)
        ...
        printk("touch buff is 0xff, need recovery state");
...
static int fts_irq_read_report(struct fts_ts_data *ts_data)
{
    ...
    case TOUCH_DEFAULT:
        ...
        if (event_num == 0) {
            printk("no touch point information(%02x)", touch_buf[2]);
            return -EIO;
        }
    ...
    default:
        printk("unknown touch event(%d)", touch_etype);
        break;
    }
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
    ...
    printk("irq:%d, flag:%x", ts_data->irq, pdata->irq_gpio_flags);
                             ...
                             struct fts_ts_platform_data *pdata)
{
    ...
    printk("display x(%d %d) y(%d %d)", pdata->x_min, pdata->x_max,
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    printk("max touch number:%d, irq gpio:%d, reset gpio:%d",
                                ...
                                unsigned long event, void *data)
{
    ...
    blank = evdata->data;
    printk("FB event:%lu,blank:%d", event, *blank);
    switch (*blank) {
    ...
    case FB_BLANK_POWERDOWN:
        if (FB_EARLY_EVENT_BLANK == event) {
            ...
            printk("suspend: event = %lu, not care\n", event);
        }
        break;
    default:
...
static int drm_check_dt(struct device_node *np)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        of_node_put(node);
        if (!IS_ERR(panel)) {
            printk("find drm_panel successfully");
            active_panel = panel;
            return 0;
        }
    }
...
//prize add by lipengpeng 20220525 start 
 /*   struct msm_drm_notifier *evdata = data;
    ...
    blank = evdata->data;
    printk("DRM event:%lu,blank:%d", event, *blank);
    switch (*blank) {
    ...
    case DRM_PANEL_BLANK_POWERDOWN:
        ...
        break;
    default:
        printk("DRM BLANK(%d) do not need process\n", *blank);
        break;
    }
...
#if defined(CONFIG_DRM_PANEL)
//prize add by lipengpeng 20220525 start
        ret = mtk_disp_notifier_register("ft8722 Touch", &ts_data->fb_notif);
        if (ret)
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    if (ts_data->fw_loading) {
        printk("fw upgrade in process, can't suspend");
        return 0;
    }
    ...
    if (ts_data->gesture_support) {
        ...
        printk("make TP enter into sleep mode");
...
static int fts_ts_probe(struct spi_device *spi)
{
    ...
    printk("Touch Screen(SPI BUS) driver prboe...");
    spi->mode = SPI_MODE_0;
    spi->bits_per_word = 8;
    ret = spi_setup(spi);
    if (ret) {
    ...
    printk("Touch Screen(SPI BUS) driver prboe successfully");
    return 0;
}
