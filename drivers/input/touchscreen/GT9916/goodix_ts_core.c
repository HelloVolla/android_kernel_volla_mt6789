...
int prize_vtp_power_init(struct i2c_client *client)
{
	...
	if (IS_ERR_OR_NULL(vdd_vtp)) {
		printk("regulator get of vdd_vtp failed");
		ret = PTR_ERR(vdd_vtp);
		vdd_vtp = NULL;
		return ret;
	}
	...
	if (regulator_count_voltages(vdd_vtp) > 0) 
	{
        ...
        if (ret) {
            printk("vdd regulator set_vtg failed ret=%d", ret);
...
int vtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (on) {
		printk("GTP power on.");
		ret = regulator_enable(vdd_vtp);
		printk("GTP power on.ret=%d",ret);
	} else {
		printk("GTP power off.");
...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
	...
	if (strlen(ts_bdata->avdd_name)) {
		...
		printk("[goodix_ts_power_init] avdd is ok avdd = %d\n",regulator_get_voltage(core_data->avdd));
