...
//---------------------------------------------------------------------------------
#define SF_DRV_VERSION "v2.3.2-2020-11-09"
...
static int sf_probe(sf_device_t *dev)
{
    ...
    printk(KERN_ERR, "%s set beanpod isee2.7.0 uuid ok \n", __FUNCTION__);
#endif
...
static int sf_read_sensor_id(void)
{
    ...
    msleep(10);
     ...
     printk("lpp---sf_read_sensor_id start\n");
    do {
     ...
     printk("lpp---sf_read_sensor_id 11111\n");
        do {
            int ic_type = -1;
            ...
            if (ret != 0) {
                printk("SPI transfer address 0x00 = 0x%02x failed, ret = %d\n", readbuf[4], ret);
                break;
            }
     printk("lpp---sf_read_sensor_id 2222\n");
            chip_id[0] = readbuf[4];
            writebuf[0] = 0x00;
            writebuf[1] = 0x01;
            writebuf[2] = 0x00;
            ...
            if (ret != 0) {
                printk("SPI transfer address 0x01 = 0x%02x failed, ret = %d\n", readbuf[4], ret);
                break;
            }
     printk("lpp---sf_read_sensor_id 3333\n");
            chip_id[1] = readbuf[4];

            if ((chip_id[0] == 0x42) && (chip_id[1] == 0x53)) {
                ic_type = 1;
                printk("%s: found ic_type 1\n", __FUNCTION__);
            }
            else if ((chip_id[0] == 0x53) && (chip_id[1] == 0x75)) {
                ic_type = 2;
                printk("%s: found ic_type 2\n", __FUNCTION__);
            }
            else {
                ic_type = -1;
                printk("%s: unknown chip_id = 0x%02x, 0x%02x\n", __FUNCTION__, chip_id[0], chip_id[1]);
                break;
            }
     printk("lpp---sf_read_sensor_id 4444\n");
            if (ic_type == 1) {
                // I2C config 0x42
                writebuf[0] = 0x01;
                writebuf[1] = 0x42;
                ...
                if (ret != 0 || readbuf[6] != writebuf[2]) {
                    printk("SPI transfer readbuf[6] = %x failed, ret = %d\n", readbuf[6], ret);
                    break;
                }
     ...
     printk("lpp---sf_read_sensor_id 5555\n");
                // I2C config 0x40
                writebuf[0] = 0x01;
                writebuf[1] = 0x40;
                writebuf[2] = 0x80;
     ...
     printk("lpp---sf_read_sensor_id 6666\n");
                ...
                if (ret != 0 || ((readbuf[4] != 0x31) && (readbuf[4] != 0x32))) {
                    printk("SPI transfer readbuf[4] = 0x%02x failed, ret = %d\n", readbuf[4], ret);
                    break;
                }

                printk("read under display chip type %d is ok\n", ic_type);
                return 0;
            }
            else { // ic_type == 2
                writebuf[0]  = 0xaa;
                ...
                if (ret != 0 || ((readbuf[7] != 0x31) && (readbuf[7] != 0x32))) {
                    printk("SPI transfer readbuf[7] = 0x%02x failed, ret = %d\n", readbuf[7], ret);
                    break;
                }
     printk("lpp---sf_read_sensor_id 7777\n");
                printk("read under display chip type %d is ok\n", ic_type);
                return 0;
            }
        }
     ...
     printk("lpp---sf_read_sensor_id 8888\n");
        printk(" tee_spi_transfer.readbuf = 0x%02x-%02x-%02x-%02x\n", readbuf[7], readbuf[8], readbuf[9],
                readbuf[10]);
        ...
        if (((0x53 == readbuf[7]) && (0x75 == readbuf[8]) && (0x6e == readbuf[9]) && (0x57 == readbuf[10]))
            ...
            printk("read fortsense id is ok\n");
            return 0;
        }
        ...
        /* 4.detect 8211 */
        {
            ...
            do {
                ...
                if (ret != 0) {
                    printk("SPI transfer failed step 1\n");
                    continue;
                }
                ...
                if (ret != 0) {
                    printk("SPI transfer failed step 2\n");
                    continue;
                }
                ...
                if (ret != 0) {
                    printk("SPI transfer failed step 3\n");
                    continue;
                }

                if (readbuf[8] == 0x82) {
                    printk("read chip is ok: 0x%02x%02x\n", readbuf[8], readbuf[9]);
                    memset(readbuf,  0, sizeof(readbuf));
                    memset(writebuf, 0, sizeof(writebuf));
                    writebuf[0] = 0x32;
                    writebuf[1] = (uint8_t)(~0x32);
