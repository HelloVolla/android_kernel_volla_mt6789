...
static unsigned int read_id = 0x9391;
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	//add by guoyingxin
	//FF_LOGE("guoyingxing debug ff_ctl_ioctl the cmd parameter value is %d\n",cmd);
//	printk("guoyingxing debug ff_ctl_ioctl the cmd parameter value is %d\n",cmd);
    ...
    if (g_log_level <= FF_LOG_LEVEL_DBG) {
		...
		unsigned int bound = _IOC_NR(FF_IOC_GET_VERSION);
         //add by guoyingxin
	//	pr_err("guoyingxin debug1 value is %d ffversion value is %d ",_cmd,FF_IOC_GET_VERSION);
		...
		//add buyguoyingxin
//		pr_err("guoyingxin debug2 value is %d ffversion value is %d ",_cmd,FF_IOC_GET_VERSION);
...
static int __init ff_ctl_driver_init(void)
{
	...
	//id=mas_connect();
	//prize by ycj start
	sprintf(current_fingerprint_info.chip,"ft9391EC");
	//prize by ycj end
